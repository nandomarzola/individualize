<?php
	...
	$base = "//".$_SERVER['HTTP_HOST'].substr($_SERVER['PHP_SELF'],0,strrpos($_SERVER['PHP_SELF'],'/')+1);
?>
<!doctype html>
<html lang="pt">
<head>
    ...
    <link rel="stylesheet" href="//fonts.googleapis.com/css?family=Noto+Sans:400,700,400italic"  id="style-resource-4">
    <link rel="stylesheet" href="tema/css/neon.css"  id="style-resource-5">
    <!-- TEMA -->

    <style type="text/css">
        .wysihtml5-sandbox{height: 650px !important;}
    </style>
</head>
...
<body class="page-body">
    <div class="page-container">
        ...
        <div class="main-content">
                    ...
                    <ul class="list-inline links-list pull-right">
                        <li>
                            <?php
                                ...
                                $sql= $mysqli->query("SELECT * FROM adm_usuarios WHERE id='$id'");
                                $e= $sql->fetch_assoc();
                            ?>
            ...
            <?php
                    ...
                    if($pagina->tabela == 'i9_area_atuacao'){
                        $pagina->tabela = 'i9_medicos';
                    }

                    if(isset($_POST['acao_form'])):
                        # Salva dados no banco
                        if(isset($_GET['id']))
                            $db = $functions->popularCampos($modulo,$pagina->campoID,$_GET['id'],$mysqli);
                        else
                            $db = null;
                        switch ($acao_form):
                            case 'editar':
                            ...
                            case 'inserir':
                                if($functions->salvarDados($pagina->campos,$_POST,$_FILES,$pagina->campoID,$db,$tela,$banco,$pagina->tabela) > 0):
                                    $functions->printMSG("Dados inseridos com sucesso. <a href='?m=".$modulo."&t=listar'><strong>Exibir registros</strong>.</a>","sucesso");
                                    unset($_POST);
                                else:
                                    $functions->printMSG("Nenhum dado foi alterado. <a href='?m=".$modulo."&t=listar'><strong>Exibir registros</strong>.</a>","alerta");
                                endif;
                        ...
                        switch($tela):
                            case 'listar':
                                echo $functions->gerarLista($pagina->listar,$pagina->campoID,$pagina->tabela,$modulo,$pagina->campos,$mysqli);
                            break;
                            case 'deletar':
                            ...
                            default:
                                ...
                                if(isset($_GET['id'])):
                                    $db = $functions->popularCampos($modulo,$pagina->campoID,$_GET['id'],$mysqli);
                                    $formulario_pronto = $functions->gerarForm($pagina->campos,$db,$tela,$pagina->operacoes,$pagina->tabela,$pagina->campoID,$mysqli);
                                else:
                                    $formulario_pronto = $functions->gerarForm($pagina->campos,null,$tela,$pagina->operacoes,$pagina->tabela,$pagina->campoID,$mysqli);
                                endif;
                            break;
                        endswitch;
                    endif;
            ...
            <footer class="main">
                <div class="pull-right">
                    &copy; <?php echo date('Y'); ?> - Painel Gerenciador de Conteúdo
                </div>
            </footer>
        </div>
    </div>
    ...
    <script src="js/jquery.multi-select.js" id="script-resource-18"></script>
    <!-- TEMA -->
