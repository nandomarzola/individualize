<?php
  ...
  /* error_reporting(E_ALL);
	...
	class Funcoes {
		...
		# Funções
		public function gerarLista($campos,$campoID,$tabela,$modulo,$camposPagina, $mysqli){
			...
			foreach($campos as $campos_h => $campo):
				if(is_array($campo)):
					if($campo[0]=='editar'){
						...
						$tabela_gerada .= '<th>'.ucfirst($campo[0]).'</th>';
					}
				else:
			...
			$tabela_gerada .= '<tbody>';
			$sql = $mysqli->query("SELECT * FROM $tabela ORDER BY $campoID DESC");
						...
						switch($campos_b):
							case 'acao':
								foreach ($campo_b as $campos_a => $campo_a):
									if($campo_a == 'editar'):
										$tabela_gerada .= '<a href="?m='.$modulo.'&t=editar&id='.$row[$campoID].'" title="Editar" class="btn btn-blue btn-sm btn-icon icon-left"><i class="entypo-pencil"></i>Editar</a>';
									elseif($campo_a == 'deletar'):
										$tabela_gerada .= '<a style="margin-left:5px !important; margin-top:0 !important;" href="?m='.$modulo.'&t=deletar&id='.$row[$campoID].'" title="Deletar" class="btn btn-red btn-sm btn-icon icon-left"><i class="entypo-cancel"></i>Deletar</a>';
									elseif($campo_a == 'mail'):
									...
									elseif($campo_a == 'preview'):
										$tabela_gerada .= '<a href="../preview/'.$row['slug'].'" target="_blank" title="Preview" class="btn btn-green btn-sm btn-icon icon-left" style="margin-top:5px;"><i class="entypo-eye"></i>Preview</a>';
									endif;
								endforeach;
							break;
							default:
								...
								//echo $query_c.'<br><br>';
								$sql_c = $mysqli->query($query_c);
								$row_c = $sql_c->fetch_array(MYSQLI_ASSOC);
		...
		public function gerarForm($campos,$db,$acao,$operacoes,$tabela,$campoID,$mysqli){
			if(in_array($acao,$operacoes)){
				...
				if($acao == 'editar' && $db == null){
					...
					foreach($campos as $campos => $campo):
						$formulario .= $this->gerarCampos($campos,$campo,$db,$mysqli);
					endforeach;
		...
		public function popularCampos($modulo,$campoID,$id,$mysqli){

			$sql = $mysqli->query("SELECT * FROM $modulo WHERE $campoID = '$id'");
			$row = $sql->fetch_array();
			return $row;
		}

		public function gerarCampos($campos,$campo,$db,$mysqli){
			...
			$retorno .= '<label for="field-1" class="col-sm-2 control-label">'.$campo[0].'</label><div class="col-sm-10">';
			switch($campo[1]):
				default:
				...
				case 'input':
					if($campo[4] == 'number')
						...
						$retorno .= '<input '.$dica.$sufixo.' class="form-control" name="'.$campos.'" maxlength="'.$campo[2].'" placeholder="'.$campo[3].'" value="'.$db[$campos].'" type="'.$campo[4].'">';
				break;
				case 'input-preco':
				...
				break;
				case 'textarea2':
					$retorno .= '<textarea '.$dica.$sufixo.' class="form-control" name="'.$campos.'" maxlength="'.$campo[2].'" placeholder="'.$campo[3].'" style="height:200px;">'.$db[$campos].'</textarea>';
				break;
				case 'select':
					if($campo[5]):
					...
					if(is_array($campo[2])):
						...
						foreach($campo[2] as $titulo => $valor):
							...
							$sql = $mysqli->query("SELECT * FROM $valor");
						...
						$sql =  $mysqli->query("SELECT * FROM $campo[2] ORDER BY nome");
						while($row =  $sql->fetch_array()):
							if($campo[5]):
								if(in_array($row[$campo[3]],explode(",",$db[$campos]))) $select = "selected"; else $select = "";
							else:
		...
		public function uploadImagem($arquivo,$pasta,$width,$height){
			if (!empty($arquivo["name"])){
				if(!preg_match("/^image\/(pjpeg|jpeg|jpg|png|gif|bmp)$/", $arquivo["type"])){
				   $error[1] = "O arquivo não é reconhecido como imagem.";
				}
				if(!isset($error)){
					...
					if($width && $height):
						$imagem_redim = $this->reajustaImagem($pasta,$nome_imagem,$caminho_imagem,$width,$height);
					else:
						$imagem_redim = $nome_imagem;
					endif;
				}
		 		if(isset($error)){
					if (count($error) != 0) {
